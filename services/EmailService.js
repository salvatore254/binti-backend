...
class EmailService {
  constructor() {
    ...
    console.log('[EMAIL] Admin email:', this.adminEmail || 'NOT CONFIGURED');
  }
  ...
  sanitizeSubject(value, fallback = '') {
    const sanitized = String(value ?? fallback).replace(/[\r\n]+/g, ' ').trim();
    return sanitized || fallback;
  }

  sanitizeBookingForHtml(booking) {
    return {
      ...booking,
      fullname: this.escapeHtml(booking.fullname || 'Valued Customer'),
      phone: this.escapeHtml(booking.phone || 'N/A'),
      email: this.escapeHtml(booking.email || 'N/A'),
      venue: this.escapeHtml(booking.venue || 'N/A'),
      setupTime: this.escapeHtml(booking.setupTime || 'N/A'),
      location: this.escapeHtml(booking.location || 'N/A'),
      additionalInfo: this.escapeHtmlWithBreaks(booking.additionalInfo || ''),
    };
  }
  ...
  getPaymentSummary(booking) {
    ...
    return {
      ...
      nextStepCopy: remainingAmount === 0
        ? 'Your booking is fully paid and confirmed.'
        : 'Your booking deposit has been received. The remaining balance will be due before your event.',
    };
  }
  ...
  async sendBookingConfirmation(booking) {
    try {
      ...
      const bookingReference = buildPublicBookingReference(booking.id || booking._id);
      ...
      const data = await this._send({
        to: booking.email,
        subject: this.sanitizeSubject(`Booking Confirmation - Binti Events (Ref: ${bookingReference})`, 'Booking Confirmation - Binti Events'),
        html: this.getBookingConfirmationTemplate(booking, depositAmount, remainingAmount),
      });

      console.log('[EMAIL] Booking confirmation sent to:', booking.email, '(Message ID:', data.id, ')');
  ...
  async sendAdminNotification(booking, depositAmount) {
    try {
      ...
      const bookingReference = buildPublicBookingReference(booking._id || booking.id);
      ...
      const data = await this._send({
        to: this.adminEmail,
        subject: this.sanitizeSubject(`New Booking Received - ${booking.fullname} (Ref: ${bookingReference})`, 'New Booking Received'),
        html: this.getAdminNotificationTemplate(booking, deposit),
      });

      console.log('[EMAIL] Admin notification sent to:', this.adminEmail, '(Message ID:', data.id, ')');
      return { success: true, messageId: data.id };
    } catch (err) {
  ...
  async sendContactMessage(name, email, phone, message, subject) {
    try {
      ...
      const safePhone = this.escapeHtml(phone || 'N/A');
      const safeMessage = this.escapeHtmlWithBreaks(message);

      const data = await this._send({
        to: this.adminEmail,
        subject: this.sanitizeSubject(subject || `New enquiry from ${name}`, 'New enquiry from website'),
        html: `<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;color:#333;background:#f5f5f5;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:20px 0;"><tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:4px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);"><tr><td style="padding:25px 30px;border-bottom:3px solid #FFC0FA;"><img src="https://bintievents.vercel.app/images/logo1.png" alt="Binti Events" width="120" style="display:block;max-width:120px;height:auto;"></td></tr><tr><td style="padding:25px 30px;"><h3 style="color:#7851A9;margin:0 0 16px 0;font-size:16px;">New Enquiry from Website</h3><table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;margin-bottom:18px;"><tr><td style="padding:8px 0;color:#666;border-bottom:1px solid #f0f0f0;width:25%;">Name</td><td style="padding:8px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;">${safeName}</td></tr><tr><td style="padding:8px 0;color:#666;border-bottom:1px solid #f0f0f0;">Email</td><td style="padding:8px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;">${safeEmail}</td></tr><tr><td style="padding:8px 0;color:#666;">Phone</td><td style="padding:8px 0;color:#333;font-weight:600;">${safePhone}</td></tr></table><h4 style="color:#7851A9;font-size:13px;margin:0 0 8px 0;">Message:</h4><p style="font-size:13px;color:#333;line-height:1.6;background:#fafafa;padding:14px;border-radius:3px;border-left:3px solid #FFC0FA;margin:0;">${safeMessage}</p></td></tr><tr><td style="background:#fafafa;border-top:2px solid #FFC0FA;padding:16px 30px;text-align:center;font-size:10px;color:#999;"><strong style="color:#666;">Binti Events Admin</strong> - Automated notification</td></tr></table></td></tr></table></body></html>`,
      });

      console.log('[EMAIL] Contact message sent (Message ID:', data.id, ')');
      return { success: true, messageId: data.id };
    } catch (err) {
  ...
  async sendPaymentConfirmation(booking, transactionId) {
    try {
      ...
      const data = await this._send({
        to: booking.email,
        subject: this.sanitizeSubject(`Payment Received - Booking Confirmed (Ref: ${bookingReference})`, 'Payment Received - Booking Confirmed'),
        html: this.getPaymentConfirmationTemplate(booking, transactionId),
      });

      console.log('[EMAIL] Payment confirmation sent to:', booking.email, '(Message ID:', data.id, ')');
      return { success: true, messageId: data.id };
    } catch (err) {
  ...
  async sendEmailWithAttachment(options) {
    try {
      ...
      console.log('[EMAIL] Email with attachment sent to:', options.to, '(Message ID:', data.id, ')');
      return { success: true, messageId: data.id };
    } catch (err) {
  ...
  async sendEmailWithHTML(options) {
    try {
      ...
      console.log('[EMAIL] HTML email sent to:', options.to, '(Message ID:', data.id, ')');
      return { success: true, messageId: data.id };
    } catch (err) {
  ...
  /**
   * Generate tent display name from tent config
   */
  ...
  /**
   * Generate tent & add-ons HTML rows for email templates
   */
  getTentAndAddonsHtml(booking) {
    let rows = '';
    // Tent configurations
    if (booking.tentConfigs && booking.tentConfigs.length > 0) {
      booking.tentConfigs.forEach((tent, idx) => {
        const name = this.escapeHtml(this.getTentDisplayName(tent));
        const qty = tent.quantity || 1;
        rows += `<tr><td style="padding:9px 0;color:#666;border-bottom:1px solid #f0f0f0;width:40%;">${name}</td><td style="padding:9px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;text-align:right;">x${qty}</td></tr>`;
      });
    } else if (booking.tentType) {
      let name = this.escapeHtml(this.getTentDisplayName({ tentType: booking.tentType, tentSize: booking.tentSize, sections: booking.sections }));
      rows += `<tr><td style="padding:9px 0;color:#666;border-bottom:1px solid #f0f0f0;width:40%;">${name}</td><td style="padding:9px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;text-align:right;">x1</td></tr>`;
    }
    // Add-ons
    const addOns = [
      { key: 'lighting', label: 'Ambient Lighting' },
      { key: 'pasound', label: 'PA Sound System' },
      { key: 'dancefloor', label: 'Dance Floor' },
      { key: 'stagepodium', label: 'Stage & Podium' },
      { key: 'welcomesigns', label: 'Welcome Signs' },
      { key: 'decor', label: 'Decor' },
    ];
    addOns.forEach(a => {
      if (booking[a.key]) {
        rows += `<tr><td style="padding:9px 0;color:#666;border-bottom:1px solid #f0f0f0;">${a.label}</td><td style="padding:9px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;text-align:right;">Included</td></tr>`;
      }
    });
    // Transport
    if (booking.transportArrangement === 'arrange' || (booking.breakdown && booking.breakdown.transport && booking.breakdown.transport.cost > 0)) {
      rows += `<tr><td style="padding:9px 0;color:#666;border-bottom:1px solid #f0f0f0;">Transport</td><td style="padding:9px 0;color:#333;font-weight:600;border-bottom:1px solid #f0f0f0;text-align:right;">Included</td></tr>`;
    }
    return rows;
  }
  ...
  getBookingConfirmationTemplate(booking, depositAmount, remainingAmount) {
    ...
    const bookingReference = buildPublicBookingReference(bookingId);
    booking = this.sanitizeBookingForHtml(booking);
