...
class EmailService {
  ...
  getTentAndAddonsHtml(booking) {
    ...
    if (booking.tentConfigs && booking.tentConfigs.length > 0) {
      ...
      let name = this.getTentDisplayName({ tentType: booking.tentType, tentSize: booking.tentSize, sections: booking.sections });
  ...
  getPaymentConfirmationTemplate(booking, transactionId) {
    ...
    const remainingAmount = totalAmount - depositAmount;
    const bookingId = booking._id || booking.id;
    const logoUrl = 'https://bintievents.vercel.app/images/invoicelogo.png';
