...
class EmailService {
  ...
  async sendBookingConfirmation(booking) {
    try {
      ...
      const data = await this._send({
        to: booking.email,
        subject: this.sanitizeSubject(`Booking Confirmation - Binti Events (Ref: ${booking.id || booking._id || 'N/A'})`, 'Booking Confirmation - Binti Events'),
  ...
  async sendAdminNotification(booking, depositAmount) {
    try {
      ...
      const data = await this._send({
        to: this.adminEmail,
        subject: this.sanitizeSubject(`New Booking Received - ${booking.fullname} (Ref: ${booking._id || booking.id})`, 'New Booking Received'),
  ...
  async sendPaymentConfirmation(booking, transactionId) {
    try {
      ...
      const data = await this._send({
        to: booking.email,
        subject: this.sanitizeSubject(`Payment Received - Booking Confirmed (Ref: ${booking._id || booking.id})`, 'Payment Received - Booking Confirmed'),
